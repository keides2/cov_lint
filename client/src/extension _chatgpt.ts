...
export function activate(context: ExtensionContext): void {
	...
	// コードアクションプロバイダを登録
	const codeActionProvider = languages.registerCodeActionsProvider('plaintext', {
		provideCodeActions(document, range, context, token) {
			const codeActions = [];

			// コードアクションを定義
			const codeAction = new CodeAction('Fix to lower case', CodeActionKind.QuickFix);

			// コードアクションが選択されたときの処理
			codeAction.command = {
				title: 'Fix to lower case',
				command: 'extension.executeCodeAction'  // コードアクションを実行するコマンド
			};
	...
	// コードアクションを実行する関数
	context.subscriptions.push(commands.registerCommand('extension.executeCodeAction', async () => {
		// コードアクションを実行するコマンドを呼び出す
		await executeCodeAction();

	}));

	async function executeCodeAction() {
		// コードアクションのタイトルとコマンド識別子
		const title = 'Fix to lower case';
		const command = { title: title, command: 'sample.fix' };  // サーバー側で設定したコマンド識別子を指定

		// コードアクションを実行
		await commands.executeCommand(command.command);
	}

	// コードアクションを実行するコマンドを登録
